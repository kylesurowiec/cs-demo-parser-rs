...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("Usage: debug_frame_parsing <demo_path>");
    ...
    // First, let's look at the raw header data
    {
        ...
        println!(
            "Signature: {}",
            std::str::from_utf8(&header_bytes[0..8]).unwrap_or("<invalid>")
        );

        let protocol = i32::from_le_bytes([
            header_bytes[8],
            header_bytes[9],
            header_bytes[10],
            header_bytes[11],
        ]);
        let network_protocol = i32::from_le_bytes([
            header_bytes[12],
            header_bytes[13],
            header_bytes[14],
            header_bytes[15],
        ...
        let server_name_end = header_bytes[16..276]
            .iter()
            .position(|&b| b == 0)
            .unwrap_or(260);
        let server_name =
            std::str::from_utf8(&header_bytes[16..16 + server_name_end]).unwrap_or("<invalid>");
        println!("Server name: '{}'", server_name);

        // Client name at offset 276, 260 bytes
        let client_name_end = header_bytes[276..536]
            .iter()
            .position(|&b| b == 0)
            .unwrap_or(260);
        let client_name =
            std::str::from_utf8(&header_bytes[276..276 + client_name_end]).unwrap_or("<invalid>");
        println!("Client name: '{}'", client_name);

        // Map name at offset 536, 260 bytes
        let map_name_end = header_bytes[536..796]
            .iter()
            .position(|&b| b == 0)
            .unwrap_or(260);
        let map_name =
            std::str::from_utf8(&header_bytes[536..536 + map_name_end]).unwrap_or("<invalid>");
        println!("Map name: '{}'", map_name);

        // Game directory at offset 796, 260 bytes
        let game_dir_end = header_bytes[796..1056]
            .iter()
            .position(|&b| b == 0)
            .unwrap_or(260);
        let game_dir =
            std::str::from_utf8(&header_bytes[796..796 + game_dir_end]).unwrap_or("<invalid>");
        println!("Game directory: '{}'", game_dir);

        // Playback time (float) at offset 1056
        let playback_time = f32::from_le_bytes([
            header_bytes[1056],
            header_bytes[1057],
            header_bytes[1058],
            header_bytes[1059],
        ...
        let playback_ticks = i32::from_le_bytes([
            header_bytes[1060],
            header_bytes[1061],
            header_bytes[1062],
            header_bytes[1063],
        ...
        let signon_length = i32::from_le_bytes([
            header_bytes[1068],
            header_bytes[1069],
            header_bytes[1070],
            header_bytes[1071],
